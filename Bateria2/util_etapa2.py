...
def getOurSqr(im_color, num_questions_DB = None, num_alternatives_DB = None):
    ...
    #print("Todos quadrados ", len(squares))
    #print("Todos quadrados 2 ", squares)
    ...
    #print("questao ", len(question_squares))
    #print("questao 2 ", question_squares)
    ...
    #print("alternativas ", len(alternative_squares))
    #print("alternativas 2 ", alternative_squares)
    ...
    for ix, i in enumerate(squares):
        ...
        y /= 4
        means.append((x, y, ix))
    if(num_questions_DB is not None and len(question_squares) != num_questions_DB):
        question_squares.clear()
        means.sort(key=lambda x: x[0])
        for i in means[:num_questions_DB]:
            question_squares.append([squares[i[2]], getCenterOfMass(squares[i[2]])])
    #print("question_square", len(question_squares))
    ...
    for i in range(len(alternative_squares)-1, -1, -1):
        if (i == len(alternative_squares)-1):
            ...
            del alternative_squares[i]
    recalcular = False
    if num_alternatives_DB is not None:
        if num_alternatives_DB != len(alternative_squares):
            recalcular = True
            #print("AQUI TAMBEM", len(alternative_squares))
        else:
            #print("PASSOU AQUI", num_alternatives_DB)
            for ix in range(1, len(alternative_squares)):
                alt0 = alternative_squares[ix-1]
                #print(alt0)
                alt1 = alternative_squares[ix]
                #print(alt1)
                media0 = sum([alt0[0][i][0] for i in range(4)]) / 4
                media1 = sum([alt1[0][i][0] for i in range(4)]) / 4
                if abs(media0 - media1) < 150:
                    recalcular = True
                    break
            #print("SAIU DAQUI")

    # if recalcular:
    if num_alternatives_DB is not None and num_alternatives_DB != len(alternative_squares):
        means.sort(key=lambda x: x[1])
        #print("num_alternatives_DB", num_alternatives_DB)

        #print("means", means)
        alternative_squares.clear()
        for i in means[:num_alternatives_DB]:
            alternative_squares.append([squares[i[2]], getCenterOfMass(squares[i[2]])])
        #print("alternative_square2", len(alternative_squares))
    #print("alternative_square", len(alternative_squares))
...
def getAnswers(img, num_questions_DB = None, num_alternatives_DB = None ):
    def getProportion(a, b):
    ...
    n, alt, img_croped = getOurSqr(img, num_questions_DB, num_alternatives_DB)
    ...
    if len(circles) > 0:
        ...
        for i in range(len(n)):
            ...
            if len(answ_rows) == 0:
                ...
                min_ix = 0
                for i in range(1, len(answ_rows)):
                    #print("debug4", answ_rows[i])
                    #print("debug5", box_rows[i])
                    centro_quadrado_y = answ_rows[i][1][1]
                    centro_circulo_y = box_rows[i][1]
                    centro_quadrado_min_y = answ_rows[min_ix][1][1]
                    centro_circulo_min_y = box_rows[min_ix][1]
                    if abs(centro_quadrado_y - centro_circulo_y) < abs(centro_quadrado_min_y - centro_circulo_min_y):
                        min_ix = i
                answ.append(answ_rows[min_ix])
                box.append(box_rows[min_ix])
            c = 0
        #print("answ1", len(answ))
        ...
        gabarito_aluno = []
    #print("gabarito", len(gabarito_aluno    ))
